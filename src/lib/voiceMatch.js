...
// A catalog entry is either { kind: 'legacy', item } or
// { kind: 'serialized', product, units }. This keeps voiceMatch decoupled
// from which table a result came from — SearchBar builds the combined list.
export function fieldsOf(entry) {
  if (entry.kind === 'serialized') {
    const p = entry.product
    return {
      name: [p.brand, p.model, p.color].filter(Boolean).join(' '),
      brand: p.brand,
      sku: p.sku,
    }
  }
  const item = entry.item ?? entry // back-compat: also accept a raw legacy item
  return { name: item.name, brand: item.brand, sku: item.sku }
}

// Returns entries sorted by relevance. Uses two filters: an absolute threshold
...
export function findVoiceMatches(rawTranscript, entries, threshold = 0.3) {
  ...
  const scored = entries
    .map((entry) => {
      ...
      return {
        entry,
        score: Math.max(
          fieldScore(candidates, normalize(f.name)),
          fieldScore(candidates, normalize(f.brand)) * 0.85,
          fieldScore(candidates, normalize(f.sku)) * 0.7
        ),
      }
    })
...
export function debugVoiceScores(rawTranscript, entries) {
  const rawNorm = normalize(rawTranscript)
  const translitNorm = normalize(devanagariToRoman(rawTranscript))
  const candidates = [...new Set([rawNorm, translitNorm])]
  return entries
