...
export default function Dashboard() {
  const { items, tags, bills, fetchAll, fetchBills, searchItems, loading } = useStore()
  ...
  const results = searchItems(query, tagFilter)
  ...
  const todayRevenue = useMemo(
    () =>
      billsToday.reduce(
        (sum, b) =>
          sum + (b.bill_items ?? []).reduce((s, li) => s + (li.unit_price || 0) * (li.quantity || 0), 0),
        0
      ),
    [billsToday]
  )
      ...
      <div className="order-2 lg:order-8">
        ...
        {!loading && results.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 xl:grid-cols-3">
            {results.map((item, i) => (
              <div key={item.id} className="animate-pop-in" style={{ animationDelay: `${Math.min(i, 8) * 35}ms` }}>
