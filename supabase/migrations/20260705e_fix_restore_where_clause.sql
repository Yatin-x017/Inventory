-- Fixes "Restore backup" failing with:
--   DELETE requires a WHERE clause
--
-- Root cause: restore_from_backup() (20260705_backup_system.sql, extended
-- by 20260705d_restore_includes_repairs.sql) wipes every table with a bare
-- `delete from public.<table>;` before reloading it from the snapshot.
-- This project's Postgres has a safety guard that rejects any UPDATE/DELETE
-- without an explicit WHERE clause, so every one of those wipe statements
-- was rejected — the whole RPC aborted (transactionally, so nothing was
-- actually lost), but the Restore button never worked.
--
-- Fix: same function body, just with `where true` added to every wipe
-- delete so the intent (delete every row) is explicit instead of implicit.
-- Full replace-in-place — nothing else changes.
--
...
begin
  ...
  -- `where true` makes the "delete every row" intent explicit — this
  -- project's Postgres rejects bare UPDATE/DELETE statements that have no
  -- WHERE clause at all.
  delete from public.repairs where true;
  delete from public.bill_items where true;
  delete from public.bills where true;
  delete from public.customer_transactions where true;
  delete from public.customers where true;
  delete from public.device_identifiers where true;
  delete from public.inventory_units where true;
  delete from public.products where true;
  delete from public.item_tags where true;
  delete from public.item_locations where true;
